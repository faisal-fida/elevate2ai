...
class ContentWorkflow:
    ...
    async def _send_media(self, phone_number: str, media_url: str, caption: str) -> None:
        ...
        await self.whatsapp.send_media(phone_number=phone_number, media_url=media_url, caption=caption)
        self.last_message_time[phone_number] = datetime.now()
    ...
    async def process_message(self, client_id: str, message: str) -> None:
        ...
        if queue.empty() and client_id not in self.client_states:
            # Start message processor for new clients
...
async def main():
    whatsapp = WhatsApp(token=settings.WHATSAPP_TOKEN, phone_number_id=settings.WHATSAPP_PHONE_NUMBER_ID)
