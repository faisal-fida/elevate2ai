...
async def revoke_all_other_sessions(
    ...
    """
    Revoke all active sessions for the current user, except the one making this request.
    """
    token = await main_oauth2_scheme(request)
    if not token:
        raise HTTPException(
    ...
    payload = verify_token(token)
    current_session_jti = payload.get("jti")

    if not current_session_jti:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Current token does not contain a session identifier (jti). Cannot exclude current session.",
        )

    revoked_count = await SessionService.revoke_all_sessions_for_user(
        db=db,
        whatsapp_number=current_user.whatsapp_number,
        except_jti=current_session_jti,
    )

    if revoked_count > 0:
        return {
            "status": "success",
            "message": f"{revoked_count} other session(s) revoked successfully.",
        }
    else:
        return {
            "status": "success",
            "message": "No other active sessions found to revoke.",
        }
